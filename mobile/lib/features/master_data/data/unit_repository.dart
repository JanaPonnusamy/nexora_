...
import 'package:nexora_mobile/features/master_data/domain/unit.dart';

/// Offline repository for units of measure (tenant-scoped, Drift-only).
class UnitRepository implements MasterWriter {
  UnitRepository(this._db, [ConflictHandler? conflict])
      : _conflict = conflict ??
  ...
  Future<int> applyDelta(MasterDelta delta, MasterScope scope) {
    return _db.transaction(() async {
      ...
      for (final record in delta.records) {
        final dto = UnitDto.fromJson(record);

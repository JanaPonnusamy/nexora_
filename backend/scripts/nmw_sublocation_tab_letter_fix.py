"""One-off: fix Products.SubLocation on the NMW branch DB where it is exactly 'TAB'.

Where SubLocation = 'TAB' (no letter suffix), set SubLocation = 'TAB ' + first
letter of ProductName (e.g. a product named "Amoxicillin" -> 'TAB A').
...
count = cur.execute(
    "SELECT COUNT(*) FROM Products WHERE SubLocation = 'TAB'"
).fetchval()
print(f"Products with SubLocation = 'TAB': {count}")

sample = cur.execute(
    "SELECT TOP 10 ProductCode, ProductName, 'TAB ' + LEFT(ProductName, 1) AS NewSubLocation "
    "FROM Products WHERE SubLocation = 'TAB'"
).fetchall()
for row in sample:

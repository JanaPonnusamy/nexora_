...
export const nmwSalesReportService = {
  listBills: (
    ...
    if (opts.purchaseStatus) q.set('purchase_status', opts.purchaseStatus)
  ...
  purchaseEntry: (tenantId: string, billNo: string, billDate: string) =>
    api.get<NmwPurchaseEntry>(
      `${base}/bills/${encodeURIComponent(billNo)}/purchase-entry?tenant_id=${encodeURIComponent(tenantId)}&bill_date=${encodeURIComponent(billDate)}`,
    ),
